...
#[derive(Default)]
pub struct RestoreTaskMutation;

#[Object]
impl RestoreTaskMutation {
    async fn restore_task(
        ...
        if status_str != "abandoned" {
            let error = async_graphql::Error::new("Only abandoned tasks can be restored")
                .extend_with(|_, e| e.set("code", ErrorCode::ValidationFailed.as_str()));
            return Err(error);
        }

        sqlx::query("UPDATE tasks SET status = 'todo', abandoned_at = NULL, abandoned_by = NULL WHERE id = ?1")
            .bind(&id)
            .execute(pool)
            .await?;
