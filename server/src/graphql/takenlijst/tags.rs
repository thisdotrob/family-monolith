...
pub async fn create_tag(ctx: &Context<'_>, name: String) -> async_graphql::Result<Tag> {
...
pub async fn delete_tag(ctx: &Context<'_>, tag_id: String) -> async_graphql::Result<bool> {
    // Require authentication
    let _claims = match ctx.data_opt::<Arc<Claims>>() {
        Some(claims) => claims,
        None => {
