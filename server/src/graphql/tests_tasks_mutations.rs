...
mod tests {
    ...
    async fn test_complete_task_concurrency_error() {
        ...
        assert_eq!(extensions.get("code").unwrap(), &async_graphql::Value::String("CONFLICT_STALE_WRITE".to_string()));
    }
    ...
    async fn test_archived_project_write_rejection() {
        ...
        let extensions = response.errors[0].extensions.as_ref().unwrap();
        assert_eq!(extensions.get("code").unwrap(), &async_graphql::Value::String("PERMISSION_DENIED".to_string()));
    }
}
